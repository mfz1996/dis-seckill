...
public class SeckillController implements InitializingBean {
    ...
    @RequestMapping(value = "/simulateSeckill", method = RequestMethod.POST)
    @ResponseBody
    public Result<Integer> simulateSeckill(HttpServletRequest request,Model model){
        UserVo user = redisService.get(SkUserKeyPrefix.TOKEN, request.getParameter(UserServiceApi.COOKIE_NAME_TOKEN), UserVo.class);
        Boolean over = localOverMap.get(1L);
        if (over)
            return Result.error(CodeMsg.SECKILL_OVER);

        // 预减库存，同时在库存为0时标记该商品已经结束秒杀
        Long stock = redisService.decr(GoodsKeyPrefix.GOODS_STOCK, "" + 1L);
        if (stock < 0) {
            localOverMap.put(1L, true);// 秒杀结束。标记该商品已经秒杀结束
            return Result.error(CodeMsg.SECKILL_OVER);
        }

        // 判断是否重复秒杀
        // 从redis中取缓存，减少数据库的访问
        SeckillOrder order = redisService.get(OrderKeyPrefix.SK_ORDER, ":" + user.getUuid() + "_" + 1, SeckillOrder.class);
        // 如果缓存中不存该数据，则从数据库中取
        if (order == null) {
            order = orderService.getSeckillOrderByUserIdAndGoodsId(user.getUuid(), 1);
        }
